...
const registration = async function (req, res) {
  try {
    ...
    const { name, email, password, profession, phone } = data;
    ...
    if (!validator.isValid(profession)) {
      return res
        .status(400)
        .send({ status: false, massage: "please enter profession" });
    }
...
const deleteduser = async (req, res) => {
    try {
        ...
        const deluser = await userModel.findByIdAndUpdate(userId, { isDeleted: true,  }, { new: true })
        return res.status(200).send({ status: true, message: "success", data: deluser })
    } catch (error) {
        return res.status(500).send({ status: false, message: error.message })
    }
}
